...
@section('title', getTranslatedWords('edit role'))
@section('css')
                        ...
                        /*$.each(res, function(key, value) {
                            ...
                            if (!values.includes(key)) {
                                ...
                                x.closest('form').parent('.modal-body').find(
                                    ...
                                    key + '">' +
                                    value + ' ' +
                                    '<a href="#" class="delete_permission btn btn-danger d-flex justify-content-center mt-2"><i class="bx bx-message-square-x"></i></a></li>'
                                );
...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{ getTranslatedWords('home') }} /</span>
            {{ getTranslatedWords('edit role') }}</h4>

        <div class="row">
            <div class="col-md-12">

                <div class="card mb-4">
                    <h5 class="card-header">{{ getTranslatedWords('edit role') }}</h5>
                    <!-- Account -->
                        ...
                        <form enctype="multipart/form-data" id="formAccountSettings" method="POST"
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-12">
                                    <label for="email"
                                        class="form-label">{{ getTranslatedWords('name') }}</label>
                                    ...
                                    <div class="bs-stepper-header" role="tablist">
                                        ...
                                        <div class="step" data-target="#monitors-part">
                                            <button type="button" class="step-trigger" role="tab"
                                                aria-controls="monitors-part" id="monitors-part-trigger">
                                                <span class="bs-stepper-circle">1</span>
                                                <span
                                                    class="bs-stepper-label">{{ getTranslatedWords('monitors') }}</span>
                                            </button>
                                        </div>
                                        <div class="line"></div>
                                        <div class="step" data-target="#processes-part">
                                            <button type="button" class="step-trigger" role="tab"
                                                aria-controls="processes-part" id="processes-part-trigger">
                                                <span class="bs-stepper-circle">2</span>
                                                <span
                                                    class="bs-stepper-label">{{ getTranslatedWords('processes') }}</span>
                                            </button>
                                        </div>
                                        <div class="line"></div>
                                        <div class="step" data-target="#permissions-part">
                                            <button type="button" class="step-trigger" role="tab"
                                                aria-controls="permissions-part" id="permissions-part-trigger">
                                                <span class="bs-stepper-circle">3</span>
                                                <span
                                                    class="bs-stepper-label">{{ getTranslatedWords('permissions selected') }}</span>
                                            </button>
                                        </div>
                                    </div>
                                    <div class="bs-stepper-content">
                                        <div id="monitors-part" class="content" role="tabpanel"
                                            aria-labelledby="monitors-part-trigger">
                                            <div class="d-flex justify-content-end mt-2 mb-2">
                                                <a class="btn btn-primary" onclick="stepper.next()"><i
                                                        class="bx @if (App::getLocale() != 'ar') bx-chevron-right @else bx-chevron-left @endif"></i></a>
                                            </div>

                                            <nav class="sidebar-permissions card py-2 mb-4">
                                                <ul class="nav flex-column" id="nav_accordion">
                                                    <label
                                                        for="">{{ getTranslatedWords('choose one monitors group at least to show permission') }}
                                                        *</label>
                                                    @foreach (Spatie\Permission\Models\Permission::groupBy('permission_group')->pluck('permission_group')->toArray() as $permission)
                                                        <li>
                                                            <input type="checkbox" class="choose_permissions"
                                                                data-group="{{ $permission }}">
                                                            <a class="nav-link d-inline-block mb-2"
                                        ...
                                        <div id="processes-part" class="content" role="tabpanel"
                                            aria-labelledby="processes-part-trigger">
                                            <div class="d-flex justify-content-center mt-2 mb-2 justify-content-between">
                                                ...
                                                <a class="btn btn-primary" onclick="stepper.previous()"><i
                                                        class="bx @if (App::getLocale() != 'ar') bx-chevron-left @else bx-chevron-right @endif"></i>
                                                </a>
                                                <a class="btn btn-primary" onclick="stepper.next()"><i
                                                        class="bx @if (App::getLocale() != 'ar') bx-chevron-right @else bx-chevron-left @endif"></i></a>
                                            </div>
                                        ...
                                        <div id="permissions-part" class="content" role="tabpanel"
                                            aria-labelledby="permissions-part-trigger">
                                                            ...
                                                            <ul class="list-unstyled">
                                                                <li class="mb-4 d-flex justify-content-between">
                                                                    <input type="hidden" name="permissions[]"
                                                                        value="{{ $k }}">{{ $item }}
                                                                    <a href="#"
                                                                        class="delete_permission btn btn-danger"><i
