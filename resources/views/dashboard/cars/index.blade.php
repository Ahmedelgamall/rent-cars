...
@section('title', getTranslatedWords('cars'))
@section('css')
...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{ getTranslatedWords('home') }} /</span>
            {{ getTranslatedWords('cars') }}</h4>
        <!-- Basic Bootstrap Table -->
        <div class="card">
            @can('add car')
                <div class="col-12 mt-2 mb-2" @if(App::getLocale()=='ar') dir="ltr" @else dir="rtl" @endif>
                    <a href="{{ route('cars.create') }}"
                        class="float-sm-right btn btn-primary">{{ getTranslatedWords('add new') }}</a>
                </div>
            @endcan
            {{-- <h5 class="card-header">{{ getTranslatedWords('cars') }}</h5>--}}
                ...
                <table class="table" id="datatables">
                    <thead>
                        <tr>
                            ...
                            <th>{{ getTranslatedWords('category') }}</th>
                            <th>{{ getTranslatedWords('price') }}</th>
    ...
    <div class="modal fade" id="delete" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('cars.destroy', 0) }}">
                    @csrf
                    @method('delete')
