...
@section('title', getTranslatedWords('faqs'))
@section('css')
...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{getTranslatedWords('home')}} /</span> {{getTranslatedWords('faqs')}}</h4>
    <!-- Basic Bootstrap Table -->
    <div class="card">
        
        <div class="col-12 mt-2 mb-2" @if(App::getLocale()=='ar') dir="ltr" @else dir="rtl" @endif>
            <a href="{{ route('faqs.create') }}" class="float-sm-right btn btn-primary">{{getTranslatedWords('add new')}}</a>
        </div>
        
        {{--<h5 class="card-header">{{getTranslatedWords('faqs')}}</h5>--}}
        <div class="table-responsive text-nowrap">
            <table class="table" id="datatables">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>{{getTranslatedWords('question')}}</th>
...
<div class="modal fade" id="delete" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('faqs.destroy', 0) }}">
                @csrf
                @method('delete')
